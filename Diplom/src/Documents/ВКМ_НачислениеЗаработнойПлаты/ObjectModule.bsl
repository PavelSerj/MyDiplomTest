...
Процедура ОбработкаПроведения(Отказ,Режим)
	...
	Если Отказ = Ложь Тогда
		...
		РасчитатьОклад();
		РасчитатьОтпуск();  //отпуск расчитан через базу, в движении заполнил базовый период
...
Процедура РасчитатьОклад()
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Результат = ВыборкаДетальныеЗаписи.ФактическийПериодДействия / ВыборкаДетальныеЗаписи.ПериодДействия * Движение.Показатель;
			Движение.Результат = Результат;
			
			Если Движение.Процент <> 0 Тогда  //если процент не равен нулю добавим к результату данный процент
				Результат = Результат + (Результат * Движение.Процент / 100);
				Движение.Результат = Результат;
			КонецЕсли;
			
			//формирую движение по удержанию оклада
...
Процедура РасчитатьОтпуск()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Движение = Движения.ВКМ_ОсновныеНачисления[ВыборкаДетальныеЗаписи.НомерСтроки - 1];	
		Результат = (ВыборкаДетальныеЗаписи.РезультатБаза / ВыборкаДетальныеЗаписи.ОтработаноЧасовБаза) * (ВыборкаДетальныеЗаписи.ДнейОтпуска + 1);
		Движение.Результат = Результат;
		
		//Движение по удержанию
		Движение = Движения.ВКМ_Удержания.Добавить();
		Движение.ПериодРегистрации = Дата;
		Движение.ВидРасчета = ПланыВидовРасчета.ВКМ_Удержания.Налог;
		Движение.Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник; 
...
Процедура СформироватьДвижениеВзаиморасчетыССотрудником()
	...
	Запрос.Текст = 
		...
		|		ПО ВКМ_НачислениеЗаработнойПлатыОсновныеНачисления.Сотрудник = ВКМ_Удержания.Сотрудник
		|			И ВКМ_НачислениеЗаработнойПлатыОсновныеНачисления.ДатаНачала = ВКМ_Удержания.БазовыйПериодНачало
		|			И ВКМ_НачислениеЗаработнойПлатыОсновныеНачисления.ДатаОкончания = ВКМ_Удержания.БазовыйПериодКонец
		|ГДЕ
		|	ВКМ_НачислениеЗаработнойПлатыОсновныеНачисления.Ссылка = &Ссылка";
